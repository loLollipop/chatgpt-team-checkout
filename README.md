...
- 客户使用该 CDK 自动分配的优惠码时保持 24 小时规则；首次成功使用不匹配的自有优惠码后，自动释放原分配码，CDK 切换为 3 小时 / 最多 3 次成功提链模式。
...
客户 CDK 首次成功使用与自动分配码不匹配的优惠码时，会立即解除原分配关系，让未售出的原优惠码回到库存，并从该次成功时刻开始切换为最长 3 小时、最多 3 次成功提链的仅 CDK 模式。只有上游返回有效 Checkout 链接才释放和计数；参数错误、代理失败和上游失败均不会释放或计数。第 3 次成功仍返回链接，之后 CDK 自动进入 `exhausted`。仅 CDK 模式内每次成功使用的优惠码会加密写入审计记录，管理员可在 CDK 列表展开查看。新规则对旧 CDK 同样有效，升级时会自动释放已进入该模式的历史绑定。管理员通用 CDK 不受该规则限制。
