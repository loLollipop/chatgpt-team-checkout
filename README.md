...
- 客户使用后台已登记优惠码时保持 24 小时规则；首次成功使用后台外优惠码后，CDK 切换为 3 小时 / 最多 3 次成功提链模式。
...
服务端固定每个新客户 CDK 有 24 小时待激活期。首次调用校验接口时写入激活时间，并把有效期重置为从激活时刻起 24 小时；使用后台已登记优惠码时可在有效期内重复生成支付链和更换国家。首次成功使用已登记优惠码后，CDK 的结束时间会精确同步为优惠码的 `auto_delete_at`，重复提链不会延后。传入自定义次数或有效期不会改变该规则。库存不足时整批生成会被拒绝，不会留下未绑定优惠码的 CDK。

客户 CDK 首次成功使用后台外优惠码时，会从该次成功时刻开始切换为最长 3 小时、最多 3 次成功提链的受限模式。只有上游返回有效 Checkout 链接才计数；参数错误、代理失败和上游失败均不计数。第 3 次成功仍返回链接，之后 CDK 自动进入 `exhausted`。受限模式内每次成功使用的优惠码会加密写入审计记录，管理员可在 CDK 列表展开查看。管理员通用 CDK 不受该规则限制。
...
- `exhausted`：旧版单次 CDK 次数已耗尽，或外部优惠码模式已成功提链 3 次。
- `expired`：超过有效期。
- `revoked`：管理员手动吊销。
...
`promoCode` 可留空。填写后台尚未删除的登记优惠码时，生成支付链成功后会自动标记为“已售出”，完整代码不再通过优惠码库存接口返回，并在 24 小时后由 Cloudflare Cron 软删除；重复提链不会延后首次自动删除时间。填写后台外优惠码时允许生成支付链，但客户 CDK 会在首次成功后切换为 3 小时 / 3 次模式，并在管理后台保留加密审计记录。
...
- `GET /api/admin/cdks?limit=500`：CDK 可视化列表和统计；新 CDK 返回可复制明文，历史哈希记录保持脱敏，并包含外部优惠码模式的成功次数与加密审计记录。
