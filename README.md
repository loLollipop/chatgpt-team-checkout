...
- 优惠码支持 Excel、CSV、TXT 和粘贴导入；所有国家共用全球库存。客户 CDK 可选择“CDK + 优惠码”或“仅生成 CDK”两种发放方式。
...
现有部署升级时也必须先执行远程迁移再发布 Worker；`wrangler deploy` 不会自动应用 D1 迁移。
...
- `count`：单批 1–50 个。
- `label`：最长 80 字符的内部备注。
- `mode`：`with_promo` 或 `cdk_only`；省略时兼容旧客户端，默认使用 `with_promo`。

`with_promo` 会为每个新 CDK 从全球库存原子分配一条优惠码；库存不足时整批生成会被拒绝，不会留下未绑定优惠码的 CDK。`cdk_only` 不检查或占用优惠码库存，也不会写入优惠码分配关系，因此可在零库存时批量生成，交付结果只包含自助提链地址和 CDK。

两种发放方式的新客户 CDK 都有 24 小时待激活期。首次调用校验接口时写入激活时间，并把有效期重置为从激活时刻起 24 小时。`with_promo` 使用自动分配的优惠码时可在有效期内重复生成支付链和更换国家；首次成功使用该分配码后，CDK 的结束时间会精确同步为优惠码的 `auto_delete_at`，重复提链不会延后。传入自定义次数或有效期不会改变该规则。

`cdk_only` 发放方式本身不分配优惠码。客户使用自己的优惠码首次成功生成支付链后，仍按现有规则进入最长 3 小时、默认 3 次成功提链的运行时仅 CDK 模式，管理员可在有效期内追加次数。发放时保存的 `issueMode` 与成功提链后产生的运行时 `externalMode` 是两个独立字段。
...
`promoCode` 可留空。客户 CDK 首次成功生成支付链时会绑定 Session Token 内的邮箱指纹，后续只有同一邮箱账号可以继续提链；管理员通用 CDK 不受该限制。数据库不保存邮箱明文，历史客户 CDK 会在下一次成功提链时自动绑定。填写当前 CDK 自动分配的优惠码时，生成支付链成功后会自动标记为“已售出”，完整代码不再通过优惠码库存接口返回，并在 24 小时后由 Cloudflare Cron 软删除；重复提链不会延后首次自动删除时间。填写任何与当前 CDK 分配码不匹配的优惠码时仍允许生成支付链，但客户 CDK 会在首次成功后释放原分配码，并切换为 3 小时 / 3 次的仅 CDK 模式，管理后台保留加密审计记录。
...
带优惠码生成（默认兼容模式，省略 `mode` 效果相同）：

```json
{ "count": 2, "label": "客户 A", "mode": "with_promo" }
```

仅生成 CDK：

```json
{ "count": 2, "label": "客户自有码", "mode": "cdk_only" }
```
