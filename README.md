...
服务端固定每个新客户 CDK 有 24 小时待激活期。首次调用校验接口时写入激活时间，并把有效期重置为从激活时刻起 24 小时；这 24 小时内可以重复生成支付链和更换国家。首次成功使用该 CDK 绑定的优惠码后，CDK 的结束时间会精确同步为优惠码的 `auto_delete_at`，重复提链不会延后。传入自定义次数或有效期不会改变该规则。库存不足时整批生成会被拒绝，不会留下未绑定优惠码的 CDK。
